...
def train( version='v0_0', pos_sr=None, neg_sr=None):
    from generator_br import gen_sample
    import functools
    fn = 'b_2024q4_samples_v4'
    output_signature = (tf.TensorSpec(shape=(13, 264), dtype=tf.float32), tf.TensorSpec(shape=(), dtype=tf.float32))
    ...
    train_ds = tf.data.Dataset.from_generator(train_gen, output_signature=output_signature).prefetch(batch_size * 20).batch(batch_size)
    test_ds = tf.data.Dataset.from_generator(test_gen, output_signature=output_signature).prefetch(batch_size * 20).batch(batch_size)
...
def predict(data_tf, out_file, version):
    model = HeteroTransformer(sn_tp=g.sn_tp, fn_voc_size=fn_voc_size, dnn_dim=32, num_hidden_layers=4,
                              num_attention_heads=4, size_per_head=8, mode='oot', fns_tp=g.fns_tp).model
    model.load_weights('saved_model/model_{}.h5'.format(version))
    #pdb.set_trace()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:  # 带参数为提取样本+训练: 原始样本文件、训练起止日期、oot起止日期、模型版本标识、使用gpus号
        ...
        if mode.find('predict') >= 0:
            predict(data_tf='train_data_{}.tf'.format(ver), out_file='score_train_{}_v1_br_ep11'.format(ver), version='b_2024q4_br')
